...
class RedisConfig(BaseSettings):
    ...
    stream: str
    group: str
    consumers: list[str] | None = None  # List of worker names
    ttl: int = 3600
...
class WorkerConfig(BaseSettings):
    ...
    @classmethod
    def _load_workers(cls, raw: dict[str, Any]) -> list["WorkerConfig"]:
        """
        ...
        for _, worker_values in worker_section.items():
    ...
    @staticmethod
    def filter(
        workers: list["WorkerConfig"], args_workers: list[str] | None
    ) -> list["WorkerConfig"]:
        """
        args > env > config
        """
