...
from redis_to_postgres.observability import Metrics
...
class PostgresManager:
    ...
    async def create(
        ...
        """
        attempt = 0
        with tracer.start_as_current_span("create_postgres_pool") as span:
            ...
            while True:
                try:
                    ...
                    return cls(config, tracer, pool=pool)
...
class RedisManager:
    def __init__(
        ...
        self.tracer = tracer
        self.redis = redis_client  # aioredis client instance
    ...
    async def xreadgroup(self, consumer: str, count: int, block: int, **kwargs):
